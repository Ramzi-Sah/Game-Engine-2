...
void ModelLoader::createModel(std::string modelName, std::string modelPath) {
    // /*
    std::string objFile = "data/models/" + modelPath + "/" + modelName + ".obj";
    std::string mtlFile = "data/models/" + modelPath + "/" + modelName + ".mtl";
    ...
    if (!file_mtl.good()) {
        ...
        while(getline(file_mtl, line)) {
            ...
            } else if (materialAttrib == "Ka") {
                // get ambient color
                glm::vec3 ambient;
                ss >> ambient.r >> ambient.g >> ambient.b;

                // set ambient color to materail
            ...
            } else if (materialAttrib == "Ks") {
                // get specular color
                glm::vec3 specular;
                ss >> specular.r >> specular.g >> specular.b;

                // set specular color to materail
            ...
            if (vertexAttrib == "v") {
                ...
                if (newMeshGroup) {
                    ...
                    if (meshGroupeMaterialName != SAH_NULL_STR) {
                        model->loadVertecies(indecies, nbrIndecies, materials[meshGroupeMaterialName]);
                    } else {
                        model->loadVertecies(indecies, nbrIndecies, Material());
                    };
