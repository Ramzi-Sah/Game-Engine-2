...
unsigned int TextureLoader::createTexture(std::string textureName, std::string filePath, bool AlphaChanel) {
    ...
    // std::cout << "image " << textureName << " width: " << width << " height: " << height << " nrChannels: " << nrChannels << '\n';
    if (!data) {
...
void TextureLoader::addTexture(std::string _textureName, unsigned int _texture) {
    m_textures[_textureName] = _texture;
};
